            ...
            <tr>
                <c:choose>
                    <c:when test="${page.firstPage}">
                        <td><a href="#" onclick="turnPage('${pageSize}', 1,this);" title="首页">首页</a></td>
                        <td><a title="上一页">&lt;&lt;</a></td>
                    </c:when>
                    <c:otherwise>
                        <td><a href="#" onclick="turnPage('${pageSize}', 1,this);" title="首页">首页</a></td>
                        <td><a href="#" onclick="turnPage('${pageSize}', ${current - 1}, this);" title="上一页">&lt;&lt;</a></td>
                    </c:otherwise>
                </c:choose>

                <c:forEach begin="1" end="${begin == 1 ? 0 : 2}" var="i">
                    <td <c:if test="${current == i}"> class="active"</c:if>>
                        <a href="#" onclick="turnPage('${pageSize}', ${i}, this);" title="第${i}页">${i}</a>
                    </td>
                </c:forEach>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <td><a href="#" onclick="turnPage('${pageSize}', ${current + 1}, this);" title="下一页">&gt;&gt;</a></td>
                        <td><a href="#" onclick="turnPage('${pageSize}', ${page.totalPages}, this);" title="尾页">尾页</a></td>
                    </c:otherwise>
                </c:choose>
